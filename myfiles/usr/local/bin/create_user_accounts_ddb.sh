...
/usr/local/bin/pull_caccounts_ddb.sh
...
IMPORT_FILE="/tmp/caccounts_ddb.txt"
IMPORT_FILE_PREV="/tmp/caccounts_ddb.txt.prev"
FLOCK_FILE="/tmp/caccounts_ddb.lock"

{
    ...
    do
      echo "##############"
      echo "$line"
      USERNAME=$( echo $line | cut -f2 -d',' | tr -d '"' )
      PASSWORD=$( echo $line | cut -f1 -d',' | tr -d '"' )
      SSH_KEY=$( echo $line | cut -f3 -d','  | tr -d '"' )
      echo "user = $USERNAME, password = $PASSWORD, ssh_key = $SSH_KEY"
      if [ -z "${PASSWORD}" ] && [ -z "${SSH_KEY}" ]
      then
      ...
      then
            ...
            continue
      elif [ "${USERNAME}" == "testyuser_ignore" ]
      then
            echo "Username not allowed. Continue"
            continue
